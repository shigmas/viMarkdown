...
bool MarkdownEditor::isInLinkURL(int pos, int& closeIX) const {
	auto* doc = document();
    QTextBlock block = doc->findBlock(pos);
...
int MarkdownEditor::countCharUntil(QTextBlock block, int pos, QChar ch) const	//	pos の ch が見出し行先頭から何番目かを計算
{
