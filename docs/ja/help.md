...
## ■ はじめに
- viMarkdown とは
  - viMarkdownは、軽快な動作と独自の拡張機能を備えた、エディタ・プレビュー連動型のMarkdownエディタです。
  - viMarkdown 独自機能：
    - エディタ側で編集可能なだけでなく、プレビュー側でも基本的な編集操作（文字挿入・削除・選択・Cut&Paste）が可能です。
    - 通常の表組み（GFM）に加え、他アプリとの連携を楽にするCSV形式の表をサポート
    - クラス図・UIモックアップ等を簡単に作図可能な罫線ブロックサポート
- 本稿の目的：
viMarkdown の基本的な操作方法を説明します。
...
**A.**, 現在（ver 0.2以下）は、**vi コマンドに対応していません**。ver 0.3 から対応予定です。
